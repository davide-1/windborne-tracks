import { NextRequest, NextResponse } from "next/server";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

type FlightPoint = { id: string; t: number; lat: number; lon: number };
type UnknownRow = Record<string, unknown> | unknown[];

const HOURS = Array.from({ length: 24 }, (_, i) => String(i).padStart(2, "0"));
const BASE = "https://a.windbornesystems.com/treasure";

const R = 6371;
const toRad = (d: number) => (d * Math.PI) / 180;
function speedKmH(a: FlightPoint, b: FlightPoint) {
  const dLat = toRad(b.lat - a.lat);
  const dLon = toRad(b.lon - a.lon);
  const s =
    Math.sin(dLat / 2) ** 2 +
    Math.cos(toRad(a.lat)) * Math.cos(toRad(b.lat)) * Math.sin(dLon / 2) ** 2;
  const dist = 2 * R * Math.asin(Math.min(1, Math.sqrt(s)));
  const dtH = Math.max(1e-6, b.t - a.t) / 3600;
  return dist / dtH;
}
const wrapLon = (x: number) => ((x + 540) % 360) - 180;

const isFiniteNumber = (v: unknown): v is number =>
  typeof v === "number" && Number.isFinite(v);

const toNumber = (v: unknown): number | null => {
  if (isFiniteNumber(v)) return v;
  if (typeof v === "string") {
    const n = Number(v);
    return Number.isFinite(n) ? n : null;
  }
  return null;
};

function salvageRows(json: unknown): UnknownRow[] {
  if (Array.isArray(json)) return json as UnknownRow[];

  if (
    typeof json === "object" &&
    json !== null &&
    "data" in json &&
    Array.isArray((json as Record<string, unknown>).data)
  ) {
    return (json as Record<string, unknown>).data as UnknownRow[];
  }

  if (typeof json === "object" && json !== null) {
    const all: UnknownRow[] = [];
    for (const v of Object.values(json)) {
      if (Array.isArray(v)) all.push(...(v as UnknownRow[]));
    }
    if (all.length) return all;
  }
  return [];
}

function parseAllArraysFromText(text: string): UnknownRow[] {
  const arrays: UnknownRow[] = [];
  const re = /\[[\s\S]*?\]/g;
  const matches = text.match(re);

  if (matches) {
    for (const m of matches) {
      try {
        const parsed = JSON.parse(m);
        if (Array.isArray(parsed)) arrays.push(...(parsed as UnknownRow[]));
      } catch {
        /* ignore */
      }
    }
  }

  if (arrays.length === 0) {
    const lines = text.split(/\r?\n/);
    for (const line of lines) {
      const s = line.trim();
      if (!s || s[0] !== "{") continue;
      try {
        const obj = JSON.parse(s);
        arrays.push(obj as UnknownRow);
      } catch {
        /* ignore */
      }
    }
  }

  return arrays;
}

// Try to pull {lat, lon, t, id} from either arrays or objects
function coercePoint(
  row: UnknownRow,
  fallbackT: number,
  hh: string,
  idx: number
): FlightPoint | null {
  // Case A: array like [lat, lon, maybeId]
  if (Array.isArray(row)) {
    const [a, b, c] = row;
    const lat = toNumber(a);
    const lon0 = toNumber(b);
    if (lat !== null && lon0 !== null) {
      const lon = wrapLon(lon0);
      if (lat >= -90 && lat <= 90 && lon >= -180 && lon <= 180) {
        const id = c != null ? String(c) : `arr-${hh}-${idx}`;
        return { id, t: fallbackT, lat, lon };
      }
    }
    return null;
  }

  // Case B: object with keys
  const obj = row as Record<string, unknown>;

  const latKeys = ["lat", "latitude", "Lat", "y"] as const;
  const lonKeys = ["lon", "lng", "longitude", "Long", "x"] as const;

  let lat: number | null = null;
  for (const k of latKeys) {
    if (lat === null && obj[k] != null) {
      const v = toNumber(obj[k]);
      if (v !== null) lat = v;
    }
  }

  let lon: number | null = null;
  for (const k of lonKeys) {
    if (lon === null && obj[k] != null) {
      const v = toNumber(obj[k]);
      if (v !== null) lon = wrapLon(v);
    }
  }

  if (lat === null || lon === null) return null;
  if (lat < -90 || lat > 90 || lon < -180 || lon > 180) return null;

  // time (prefer valid epoch seconds; else fallback to file hour)
  const timeCands = [
    obj.t,
    obj.ts,
    obj.time,
    obj.timestamp,
    obj.date,
    obj.dt,
    obj.epoch,
    obj.ms,
  ];
  let t: number | null = null;
  for (const c of timeCands) {
    if (c == null) continue;

    if (isFiniteNumber(c)) {
      t =
        c > 1e12 ? Math.floor(c / 1000) : c > 1e9 && c < 4e9 ? Math.floor(c) : null;
      if (t) break;
    }

    if (typeof c === "string") {
      const num = Number(c);
      if (Number.isFinite(num)) {
        t =
          num > 1e12
            ? Math.floor(num / 1000)
            : num > 1e9 && num < 4e9
            ? Math.floor(num)
            : null;
        if (t) break;
      }
      const d = Date.parse(c);
      if (!Number.isNaN(d)) {
        t = Math.floor(d / 1000);
        break;
      }
    }
  }
  if (!t) t = fallbackT;

  // id
  const id =
    [obj.id, obj.i, obj.name, obj.device_id, obj.balloon_id, obj.serial, obj.unit_id]
      .map((v) => (v == null ? null : String(v)))
      .find((s) => Boolean(s)) || `obj-${hh}-${idx}`;

  return { id, t, lat, lon };
}

async function fetchHour(hh: string): Promise<FlightPoint[]> {
  const url = `${BASE}/${hh}.json`;
  const ac = new AbortController();
  const to = setTimeout(() => ac.abort(), 12000);
  try {
    const res = await fetch(url, {
      signal: ac.signal,
      cache: "no-store",
      headers: {
        Accept: "application/json,text/plain;q=0.8,*/*;q=0.5",
        "User-Agent": "Mozilla/5.0 (compatible; WindborneChallenge/1.0)",
      },
    });
    const text = await res.text();

    let rows: UnknownRow[] = [];
    try {
      rows = salvageRows(JSON.parse(text));
    } catch {
      rows = parseAllArraysFromText(text);
    }

    // Timestamp: align to "hh hours ago" from NOW
    const fallbackT = Math.floor(Date.now() / 1000) - parseInt(hh, 10) * 3600;

    const out: FlightPoint[] = [];
    rows.forEach((row, i) => {
      const p = coercePoint(row, fallbackT, hh, i);
      if (p) out.push(p);
    });
    return out;
  } catch {
    return [];
  } finally {
    clearTimeout(to);
  }
}

export async function GET(_req: NextRequest) {
  const results = await Promise.all(HOURS.map(fetchHour));
  const all: FlightPoint[] = results.flat();

  // group → sort → relaxed teleport filter
  const byId = new Map<string, FlightPoint[]>();
  for (const p of all) {
    const a = byId.get(p.id) ?? [];
    a.push(p);
    byId.set(p.id, a);
  }

  const balloons = [...byId.entries()]
    .map(([id, pts]) => {
      const sorted = pts.slice().sort((a, b) => a.t - b.t);

      // de-dup exact duplicates
      const dedup: FlightPoint[] = [];
      for (const p of sorted) {
        const prev = dedup[dedup.length - 1];
        if (prev && prev.t === p.t && prev.lat === p.lat && prev.lon === p.lon) continue;
        dedup.push(p);
      }

      // allow up to 1000 km/h to avoid over-pruning
      const cleaned: FlightPoint[] = [];
      for (let i = 0; i < dedup.length; i++) {
        if (i === 0) {
          cleaned.push(dedup[i]);
          continue;
        }
        const v = speedKmH(dedup[i - 1], dedup[i]);
        if (Number.isFinite(v) && v < 1000) cleaned.push(dedup[i]);
      }

      const latest = cleaned.at(-1) ?? null;
      return { id, points: cleaned, latest };
    })
    .filter((b) => b.points.length >= 1);

  return NextResponse.json({ updatedAt: Date.now(), balloons });
}

